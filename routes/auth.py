...
def register():
    try:
        ...
        user = User(username=data['username'], password=hashed_password, factory_id=data['factory_id'])
...
def adminregister():
    try:
        ...
        hashed_password = generate_password_hash(data['password'])
        user = User(username=data['username'], password=hashed_password, factory_id=None, is_admin=True)
        mongo.db.users.insert_one(user.to_dict())
        return jsonify({"ok": True, "message": "User registered successfully"}), 201
    except Exception as e:
        return jsonify({"ok": False, "message": "An error occurred: " + str(e)}), 500
